    ...
    >
      ...
      {!imageError && (
        <motion.img
          ...
          animate={isCardInView ? { scale: 1, opacity: 1 } : { scale: 1.3, opacity: 0 }}
      ...
      <motion.div
        className="absolute -inset-4 rounded-3xl opacity-0 group-hover:opacity-40 pointer-events-none blur-xl"
        style={{
...
const ChefsSection = ({ chefs }) => {
  ...
  const [tooltip, setTooltip] = useState({ show: false, chef: null, x: 0, y: 0 });

  const handleMouseEnter = (chef, event) => {
    setTooltip({
      show: true,
      chef,
      x: event.clientX,
  ...
  const handleMouseLeave = () => {
    setTooltip({ show: false, chef: null, x: 0, y: 0 });
  };
    ...
    >
      ...
      <AnimatePresence>
        {tooltip.show && tooltip.chef && (
          ...
          >
            <div className="bg-white/95 backdrop-blur-sm rounded-xl px-4 py-3 shadow-xl border border-gray-200 max-w-xs">
...
const ChefCard = ({ chef, variants, onClick, onMouseEnter, onMouseLeave }) => {
  const [isHovered, setIsHovered] = useState(false);
    ...
    >
      ...
      >
        ...
        <div className="relative p-8">
          <motion.h3 
            className="text-2xl md:text-3xl font-light text-white mb-2 tracking-wide hover:text-amber-400 transition-colors"
